...
class CharCorruptionDataset(Dataset):
    def __init__(self, data, block_size):
        ...
        self.itos = { i:ch for i,ch in enumerate(chars) } # len = 256
    ...
    def __getitem__(self, idx):
        ...
        truncated_doc = document[:random.randint(4, self.max_context_size)]

        length = max(1, min(len(truncated_doc)-2, int(round(random.gauss(len(truncated_doc) * self.masking_percent, 5)))))
        length_fix = (len(truncated_doc) - length) // 2
        if (length + length_fix*2) != len(truncated_doc):
            length += 1
        prefix = truncated_doc[:length_fix]
        masked_content = truncated_doc[length_fix:(length_fix + length)]
        suffix = truncated_doc[(length_fix + length):(length_fix*2 + length)]
        
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR
        n = self.block_size - len(masked_string)
        masked_string = masked_string + (self.PAD_CHAR * n)

        x = masked_string[:-1]
        y = masked_string[1:]

        long_x = torch.tensor([self.stoi[char] for char in list(x)])
        long_y = torch.tensor([self.stoi[char] for char in list(y)])
