...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_split = list(torch.split(Y,1,dim=0))
        for y in Y_split:
            y_squeezed = torch.squeeze(y,dim=0)
            ybar_t = torch.cat((y_squeezed,o_prev), dim=1)
            dec_new_state, combined_output, e_t = self.step(ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(combined_output)
            o_prev = combined_output
            dec_state = dec_new_state
