...
def finetune(reading_params_path, finetune_corpus_path, pretrain_dataset, block_size, model, finetune_lr=6e-4, writer=None):
    ...
    if reading_params_path is not None: 
        ...
        tconf = TrainerConfig(max_epochs=10, batch_size=256, learning_rate=finetune_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=0)
    else:
        tconf = TrainerConfig(max_epochs=75, batch_size=256, learning_rate=finetune_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=0)
...
def pretrain(pretrain_dataset, block_size, model, pretrain_lr=6e-3, writer=None):
    ...
    ### START CODE HERE
    tconf = TrainerConfig(max_epochs=650, batch_size=128, learning_rate=pretrain_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=0)
...
def train(model, writing_params_path, trainer_obj):
    ...
    ### START CODE HERE
    trainer_obj.train()
    state_dict = trainer_obj.model.state_dict()
    new_state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
    torch.save(new_state_dict, writing_params_path)
    ### END CODE HERE
    return
