...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  y_hat = softmax(outside_vectors @ center_word_vec.T)
  number_of_words = y_hat.shape[0]
  y_hat = y_hat.reshape((number_of_words,1))
  y_hat_o = softmax(outside_vectors @ center_word_vec.T)[outside_word_idx]
  y = np.zeros((number_of_words,1))
  y[outside_word_idx] += 1
  loss = -np.log(y_hat_o)
  grad_center_vec = (outside_vectors.T @ (y_hat - y)).reshape(-1)
  grad_outside_vecs = (y_hat - y) @ center_word_vec.reshape((1,center_word_vec.shape[0]))
  ### END CODE HERE
...
def skipgram(current_center_word, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for index, word in enumerate(outside_words):
     loss_current, gradc, grado = word2vec_loss_and_gradient(v_c,word2ind[word],outside_vectors,dataset)
     loss += loss_current
     grad_center_vecs[center_word_index] += gradc
