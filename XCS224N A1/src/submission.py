...
def distinct_words(corpus):
    ...
    # ### START CODE HERE ###
    for list_of_string in corpus:
        for word in list_of_string:
            if word not in corpus_words:
                corpus_words.append(word)
    corpus_words = sorted(corpus_words, key=None, reverse=False)
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    for list_of_string in corpus:
        for index, word in enumerate(list_of_string):
            past_n = list_of_string[max(index - window_size, 0):index]
            next_n = list_of_string[index+1:min(index + window_size+1,len(list_of_string))]
            co_occurence = past_n + next_n
            for co_word in co_occurence:
...
def reduce_to_k_dim(M, k=2):
    ...
    # ### START CODE HERE ###
    svd = TruncatedSVD(k, algorithm='randomized',n_iter=10,random_state=np.random.seed(4355))
