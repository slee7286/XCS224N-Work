...
def precompute_rotary_emb(dim, max_positions):
    ...
    angles = torch.tensor([[t*theta_i for theta_i in theta] for t in range(max_positions)])
    cos = torch.cos(angles)
    sin = torch.sin(angles)
    rope_cache = torch.stack((cos,sin),dim=2)
    ### END CODE HERE
    return rope_cache
...
def apply_rotary_emb(x, rope_cache):
    ...
    ### START CODE HERE
    t = x.shape[2]
    rope_cache_t = rope_cache[:t,:,:]
    rope_com = torch.view_as_complex(rope_cache_t)
    rope_com_reshape = rope_com.reshape(t, x.shape[1], x.shape[3] // 2)
    rope_com_rere = rope_com_reshape.permute(1,0,2)
    x_reshaped = torch.view_as_complex(torch.stack((x[:,:,:,0::2],x[:,:,:,1::2]),dim=-1))
    rope_hadamard = rope_com_rere.unsqueeze(0).unsqueeze(0) * x_reshaped
    rotated_x = torch.view_as_real(rope_hadamard).flatten(-2).squeeze(0)
    ### END CODE HERE
    return rotated_x
